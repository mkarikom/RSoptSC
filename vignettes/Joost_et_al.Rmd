---
title: "scRNA-seq Clustering of Sking Cells"
author: "Matt Karikomi"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{scRNA-seq Clustering of Sking Cells}
...
cluster_dtree <- GetDominatorTree(cluster_predecessors)
set.seed(1)
l1 <- igraph::layout.lgl(cluster_dtree)
l1 <- igraph::layout.norm(l1, ymin = -1, ymax = 1, xmin = -1, xmax = 1)
plot(cluster_dtree,
     vertex.size=20,
     rescale = F,
     layout = l1 * 0.9,
...
Compute the directional minimum spanning tree on the cell-cell graph for visualization.  The network diagram uses Fruchterman-Reingold algorithm to ensure visually appealing separation of nodes.

```{r, results='asis', out.width = '100%', out.height = '100%', fig.width = 7, fig.heigt = 7}
cell_mst <- igraph::mst(mapped$similarity_graph)
cell_predecessors <- GetPredecessors(cell_mst, root_cell)
cell_dtree <- GetDominatorTree(cell_predecessors)
set.seed(1)
