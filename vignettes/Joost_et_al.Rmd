...
rm(list = ls())
LOCAL <- identical(Sys.getenv("LOCAL"), "true") # don't try to build on cran or travis (see: http://www.lyzander.com/r/2017/01/30/cran_travis.html)
...
```{r, results='asis', echo=FALSE,results='hide', eval=LOCAL}
...
```{r, results='asis', warning=FALSE, eval=LOCAL}
...
Cells are labeled by cluster membership and plotted on low-dimensional embedding (umap) of the similarity matrix.

```{r, results='asis', out.width = '100%', out.height = '100%', fig.width = 7, fig.heigt = 7, eval=LOCAL}
...
```{r, fig.width=7, fig.height=7, eval=LOCAL}

markers <- GetMarkerTable(counts_data = log(data+1),
...
Violin Plot of Krt14 expression.  Note strong expression of the basal compartment (clusters 4 and 5):
...
```{r, results='asis', out.width = '100%', out.height = '100%', fig.width = 7, fig.heigt = 7, eval=LOCAL}
  ...
  ViolinPlotExpression(data = logdata,
                       ...
                       gene_name = "Krt14")
```
Violin Plot of Krt10 expression.  Note uniformly high expression in the IFE-DI/DII compartments (clusters 1, 2, 7).  Note that the mixed expression of Krt10 in cluster 5, which may indicate a population of transitioning basal cells:
...
```{r, results='asis', out.width = '100%', out.height = '100%', fig.width = 7, fig.heigt = 7, eval=LOCAL}
  ...
  ViolinPlotExpression(data = logdata,
                       ...
                       gene_name = "Krt10")
```
Violin Plot of Lor expression.  Note strong expression in clusters 3 and 6 which fully encompass the terminally differentiated IFE-KI/KII compartments:
...
```{r, warning=FALSE, eval=LOCAL}
library(knitr)
library(kableExtra)

lig_rec_path <- system.file("extdata", "tgfb_lig_rec.tsv", package = "RSoptSC")
rec_target_path <- system.file("extdata", "tgfb_rec_target.tsv", package = "RSoptSC")
...
```{r, results='hide', out.width = '100%', out.height = '100%', fig.width = 7, fig.heigt = 7, message=FALSE, eval=LOCAL}
...
```{r, out.height='100%', out.width='100%', eval=LOCAL}
  par(mar=c(1,1,1,1))
...
```{r, out.width = '100%', out.height = '100%', fig.width = 7, fig.heigt = 7, eval=LOCAL}
...
```{r, results='asis', warning=FALSE, message= FALSE, out.width = '100%', out.height = '100%', fig.width = 7, fig.heigt = 7, eval=LOCAL}
  par(mar=c(1,1,1,1))
