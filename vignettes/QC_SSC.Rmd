...
library(RSoptSC)
upper <- 10000 # number of variable genes
cache <- ReadAnnotateCountsSCE(series_accession = 'GSE67602', GEOfilecache = '/tmp/', 
                               pheno_filter = c('IFE-B','IFE-DI','IFE-DII','IFE-KI','IFE-KII'),
                               annotation_db = 'org.Mm.eg.db',
                               feature_annotation_key = "SYMBOL")
# which cells are epidermis

#similarity_data <- logcounts(cache$pheno[cache$vargenes[1:upper],])
all_data <- logcounts(cache$qc)
similarity_data <- logcounts(cache$qc)
gene_names <- rownames(similarity_data)


# similarity_data <- counts(cache$filtered[1:1000,1:100])
# all_data <- counts(cache$qc[1:1000,1:100])
...
Cluster the cells by factoring the similarity matrix.  Infer cluster number using spectra of the ensemble Laplacian. In this analysis, we obtain bounds on the number of clusters, where the lower bound is equal to the number of zero eigenvalues and the upper bound is the index of the eigenvalue preceding the largest eigengap.  We take this value as the cluster number here.

```{r, results='asis', message=FALSE, warning=FALSE}
clusters <- ClusterCells(similarityMatrix = S$W, n_comp = 10, .options='p')
H <- clusters$H
...
```{r, results='asis', out.width = '100%', out.height = '100%', fig.width = 7, fig.heigt = 7}
true_labels <- cache$orig$Phenotype
# plot clusters
FeatureScatterPlot(flat_embedding = low_dim_mapping$flat_embedding,
                   feature = cache$qc$Phenotype,
                   title = "True Labeling",
                   subtitle = "t-SNE Embedding",
...
```{r, fig.width=7, fig.height=7}
  ...
  markers <- GetMarkerTable(counts_data = similarity_data,
                            cluster_labels = labels,
                            H = H,
                            n_sorted = 25)

p <- PlotTopN(data = similarity_data,
              cluster_labels = labels,
              markers = markers$all,
              n_features = 15)
```
...
data_gene <- similarity_data[gene_index,]
...
```{r, results='asis', out.width = '100%', out.height = '100%', fig.width = 7, fig.heigt = 7}
  plot(ViolinPlotExpression(data = similarity_data,
...
```{r, results='hide', out.width = '100%', out.height = '100%', fig.width = 7, fig.heigt = 7, message=FALSE}
  ...
  Pmats <- GetSignalingPartners(similarity_data,
                                gene_names,
...
```{r, results='asis', warning=FALSE, message= FALSE, out.width = '100%', out.height = '100%', fig.width = 7, fig.heigt = 7}
  ...
  PlotClusterExpression(data = similarity_data,
                        cluster_labels = labels, 
                        markers = gene_list)
```